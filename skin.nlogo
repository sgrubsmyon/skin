...
;firm procedure
to make-advert
  set advert map [ ?1 -> item ?1 capabilities ] ih
end
...
to-report inputs-available [ market ]
  foreach inputs [ ?1 ->
    ...
    if ?1 >= raw-materials and not any? market with [ ?1 = product ]  [ report false ]
  ]
  report true
end
...
to-report profitable [ market ]
  ...
  if not trading? [
    ...
    foreach inputs [ ?1 ->
      let supplier nobody
      ifelse ?1 >= raw-materials [
        ; find all suppliers of this input
        let possible-suppliers market with [?1 = product ]
        ; find cheapest
...
to find-partners
  ...
  set candidates (turtle-set previous-partners (filter [ ?1 -> is-firm? ?1 ] suppliers) (filter [ ?1 -> is-firm? ?1 ] customers))
...
to-report intersection [ set-a set-b ]
  let set-c []
  foreach set-a [ ?1 -> if member? ?1 set-b [ set set-c fput ?1 set-c ] ]
  report set-c
end
...
to-report set-difference [ set-a set-b ]
  ...
  foreach set-c [ ?1 -> if member? ?1 set-b [ set set-b remove ?1 set-b ] ]
  report set-b
end
...
to add-capabilities [ other-firm ]
  let my-position 0
  foreach [ih] of other-firm [ ?1 ->
    let capability item ?1 [capabilities] of other-firm
    ifelse member? capability capabilities [
      ;  capability already known to me
      set my-position position capability capabilities
      if item my-position expertises < item ?1 [expertises] of other-firm [
        set expertises replace-item my-position expertises item ?1 [expertises] of other-firm
        set abilities replace-item my-position abilities item ?1 [abilities] of other-firm
      ]
    ]
    [
...
to copy-ih-capabilities [ source ]
  foreach [ih] of source [ ?1 ->
    set capabilities fput item ?1 [capabilities] of source capabilities
    set abilities fput item ?1 [abilities] of source abilities
    set expertises fput item ?1 [expertises] of source expertises
  ]
end
...
to distribute-network-profits
  if length members > 0 and [capital] of hq > initial-capital [
    ...
    foreach members [ ?1 ->
      ask ?1 [ set capital capital + amount-to-distribute ]
    ]
...
645
28
SKIN (Simulating Knowledge Dynamics in Innovation Networks) 5.32
11
0.0
1
